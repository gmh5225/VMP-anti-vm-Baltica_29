...
namespace vmp_improve
{
	...
	namespace assembly_code
	{
		...
		EXTERN_C void abuse_step_rdtsc();
	}
	...
	namespace util
	{
		...
		INLINE  auto get_windows_number() -> INT
		{
			...
			auto NtMajorVersion = *(uint8_t*)0x7FFE026C;
	...
	{ 
		...
		EXCEPTION_POINTERS pointer_except = { 0 };
		__try
		{
#ifdef _WIN64
		...
		{
			std::cout << "opcode 1 ->\t" << std::hex << (INT)byte_step << "\n";
			if (byte_step != 0x90) //is exception address opcode is nop?
				std::cout << "Opcode 1 bad!\n";
		}
		...
		__except (byte_step = *(uint8_t*)(GetExceptionInformation())->ExceptionRecord->ExceptionAddress)
		{
			//16 17 9C F6 44 24 01 01 75 0A
			std::cout << "opcode 2 ->\t" << std::hex << (INT)byte_step << "\n";

			if (byte_step != 0x90)//is exception address opcode is nop?
				std::cout << "Opcode 2 bad!\n";
		} 
